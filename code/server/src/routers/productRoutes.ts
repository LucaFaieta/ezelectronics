...
class ProductRoutes {
    ...
    initRoutes() {
         ...
         * It requires the user to be logged in and to be a manager or a admin.
         ...
         */
        this.router.post(
            "/", this.authenticator.isLoggedIn, this.authenticator.isAdminOrManager,
            ...
            body("sellingPrice").notEmpty().isFloat({ gt: 0}),
         ...
         */
        this.router.patch(
            "/:model", this.authenticator.isLoggedIn, this.authenticator.isAdminOrManager,
         ...
         */
        this.router.patch(
            "/:model/sell", this.authenticator.isLoggedIn, this.authenticator.isAdminOrManager,
            param("model").notEmpty().isString(), 
            body("quantity").notEmpty().isInt({ min: 1 }),
         ...
         */
        this.router.get(
            "/available", this.authenticator.isLoggedIn,
            query("grouping").optional().isString().isIn(["category", "model"]),
            query("category").custom((value, { req }) => {
                if (req.query.grouping == "category") {
                    if (value == "Smartphone" || value == "Laptop" || value == "Appliance") {
